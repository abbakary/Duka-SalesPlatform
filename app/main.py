...
def _coerce_datetimes(data: dict) -> dict:
    for k, v in data.items():
        if isinstance(v, str) and (k.endswith("_at") or k.endswith("_date") or k == "expiry" or k == "start_time" or k == "end_time"):
            try:
...
def receive_purchase_order(
    ...
    for item in po.items:
        item_id = item.get("id")
        qty_received = int(receive_qtys.get(item_id, 0))
        product_id = item.get("productId") or item.get("product_id")
        
        if qty_received > 0:
            item["receivedQuantity"] = item.get("receivedQuantity", 0) + qty_received
            ...
            if auto_create_products:
                ...
                if not product:
                    ...
                    product = Product(
                        ...
                        cost_price=float(item.get("unitCost", 0)),
                        is_active=True,
                        created_at=datetime.utcnow(),
                        updated_at=datetime.utcnow(),
                    )
                ...
                # Just update the existing product quantity
                if product_id:
                    product = db.query(Product).filter(Product.id == product_id).first()
                    if product:
                        product.quantity += qty_received
    ...
    ordered_total = sum(item.get("orderedQuantity", 0) for item in updated_items)
    received_total = sum(item.get("receivedQuantity", 0) for item in updated_items)
